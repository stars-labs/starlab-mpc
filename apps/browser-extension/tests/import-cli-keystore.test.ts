...
// setup-bun.ts stashes the native WebCrypto as REAL_WEBCRYPTO before
// installing its mocks; we capture its subtle directly as a local so
// the setup-bun mock can't interfere with this AES-GCM roundtrip.
...
describe('CLI Keystore Import', () => {
    it('should decrypt and parse CLI keystore correctly', async () => {
        ...
        // Uses the real native WebCrypto subtle (captured by setup-bun
        // before it installed its mocks).
        const subtle = REAL_WEBCRYPTO.subtle;
