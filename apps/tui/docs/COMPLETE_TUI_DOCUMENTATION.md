...
- **Memory zeroization**: only
  `packages/@starlab/core/src/root_secret.rs` zeros on
...
apps/tui/examples/      # Runnable end-to-end scenarios
├── offline_dkg_demo.rs
...
exist (`find apps/tui/tests -type f` returns only the two
...
#### Development
```bash
cargo build --bin starlab-tui
RUST_LOG=debug ./target/debug/starlab-tui
```

#### Release
```bash
cargo build --release --bin starlab-tui
strip target/release/starlab-tui
```

#### Platform-Specific

**All platforms**: `cargo build --release -p starlab-client
--bin starlab-tui` produces a single static binary at
`target/release/starlab-tui`. Distribute the binary directly —
...
to live at `apps/tui/Dockerfile` was written for a pre-monorepo,
...
- Placement at the monorepo root, not under apps/tui/
- A multi-stage build that copies every workspace member crate so
  ...
  binary: `cargo build --release --bin starlab-tui -p starlab-client`

See `apps/tui/docs/DEPLOYMENT_GUIDE.md` for the currently-
...
```rust
// Real definition: apps/tui/src/elm/message.rs
pub enum Message {
...
`apps/tui/src/elm/provider.rs:24` with 24 async methods for
...
flags only. See `apps/tui/src/bin/starlab-tui.rs` for the
authoritative `clap::Args` struct, or `apps/tui/docs/README.md`
...
`packages/@starlab/core` which carries FROST-specific
...
[github.com/hecoinfo/starlab-mpc](https://github.com/hecoinfo/starlab-mpc)
