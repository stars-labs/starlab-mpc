...
**Fix Applied** (`src/bin/starlab-tui.rs`):
...
1. `/apps/tui/src/elm/components/dkg_progress.rs` - Fixed gauge percentage calculation
2. `/apps/tui/src/elm/update.rs` - Added JoinSession arrow key handling
3. `/apps/tui/src/elm/app.rs` - Fixed TTY mode handling for testing
4. `/apps/tui/src/bin/starlab-tui.rs` - Added panic handler

## How to Test

1. Run mpc-2 to join a DKG session:
   ```bash
   cargo run --bin starlab-tui -- --device-id mpc-2
   ```
...
4. For automated testing:
   ```bash
   FORCE_TTY=1 cargo run --bin starlab-tui -- --device-id mpc-2
   ```