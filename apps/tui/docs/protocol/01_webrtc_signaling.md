...
via `packages/@starlab/types/src/session.ts` on the TS side
...
>
>   - `WebRTCMessage<C>` — `apps/tui/src/protocal/signal.rs:204`
...
> TypeScript mirror at `packages/@starlab/types/src/webrtc.ts:26`):
>
> ```json
> {
...
`apps/tui/src/protocal/signal.rs:204`. The enum is tagged
...
`packages/@starlab/types/src/webrtc.ts:26`.
